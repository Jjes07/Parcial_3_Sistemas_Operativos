...
*
* Cumple con:
* * Compresión/descompresión LZW.
* * Cifrado/descifrado ChaCha20 con clave derivada de passphrase usando SHA-256.
* * Concurrencia (hilos POSIX) para procesar varios archivos dentro de un directorio.
* * Sin uso de <stdio.h> (fopen, fread, fwrite, fclose).
*
* Uso:
* ./gsea -c --comp-alg lzw -i <ruta_entrada> -o <ruta_salida>
* ./gsea -d --comp-alg lzw -i <ruta_entrada> -o <ruta_salida>
* ./gsea -e --enc-alg chacha20 -i <ruta_entrada> -o <ruta_salida> -k <clave(opcional)>
* ./gsea -u --enc-alg chacha20 -i <ruta_entrada> -o <ruta_salida> -k <clave(opcional)>
...
*
* Ejemplo:
* ./gsea -c --comp-alg lzw -i "pruebas/archivo.txt" -o "pruebas/-c"
* ./gsea -d --comp-alg lzw -i "pruebas/-c/archivo.txt.lzw" -o "pruebas/-d"
* ./gsea -e --enc-alg chacha20 -i "pruebas/archivo.txt" -o "pruebas/-e" -k "mi_clave_secreta"
* ./gsea -u --enc-alg chacha20 -i "pruebas/-e/archivo.txt.enc" -o "pruebas/-u" -k "mi_clave_secreta"
* ./gsea -ce --comp-alg lzw --enc-alg chacha20 -i "pruebas/archivo.txt" -o "pruebas/-ce" -k "mi_clave_secreta"
* ./gsea -du --comp-alg lzw --enc-alg chacha20 -i "pruebas/-ce/archivo.txt.lzw.enc" -o "pruebas/-du" -k "mi_clave_secreta"
*
*/
...
// =============================
//  G S E A   —   BLOQUE 1
//  Includes, utilidades, POSIX helpers, LZW completo
// =============================
...
int compress_buffer_lzw(const uint8_t *input, size_t length, int outfd) {
    ...
    // inicialización diccionario con valores 0..255
    for (int i = 0; i < 256; i++) {
        Entry e;
...
// =============================
//  G S E A   —   BLOQUE 2
//  ChaCha20, SHA-256 KDF simple, nonce & paralelización por chunks
// =============================

// -----------------------------
// SHA-256 (min implementación para derivar clave)
// Fuente adaptada y reducida (propósito: derivar 32 bytes desde passphrase).
// Implementación compacta pero correcta.
// -----------------------------
typedef struct {
    uint32_t state[8];
    uint64_t bitlen;
...
// -----------------------------
// Deriva 32 bytes de clave desde passphrase usando SHA-256(passphrase)
// Si passphrase es NULL o cadena vacía -> usamos clave por defecto.
// -----------------------------
...
//  - key: 32 bytes
//  - nonce: 12 bytes
//  - counter: 32-bit block counter (we use 64-bit logically via two counters when necessary)
//  Block size: 64 bytes
// -----------------------------
...
// -----------------------------
// XOR buffer with ChaCha20 keystream starting at given block counter.
// - offset_blocks: starting block index (each block 64 bytes)
// - len: number of bytes to xor (may not be multiple of 64)
// This function generates keystream blocks sequentially and xors into dst (in-place).
// -----------------------------
static void chacha20_xor_keystream(const uint8_t key[32], const uint8_t nonce[12],
    ...
    // Note: for files larger than 2^32 blocks you'd need 64-bit counter handling per RFC; here we assume files are smaller.
    while (pos < len) {
        uint8_t block[64];
...
// -----------------------------
// Parallel encrypt/decrypt file routine
// - in_fd: descriptor de entrada (posicionado al inicio de los datos a procesar)
// - out_fd: descriptor de salida (posicionado al lugar donde escribir)
// - key (32 bytes), nonce (12 bytes)
// - num_threads: cuantos hilos usar para dividir el trabajo
//
// Behavior:
//   - The caller is responsible for reading/writing header (we will handle in higher-level functions).
//   - This function reads all data into RAM (simple approach), splits into N chunks and spawns threads.
//   - Each thread xors its chunk with ChaCha20 keystream starting at correct block offset.
// -----------------------------
typedef struct {
    uint8_t *buf;       // pointer to the whole data buffer (owned by caller)
    size_t offset;      // start byte offset in buffer
    size_t len;         // length of chunk
    const uint8_t *key;
    const uint8_t *nonce;
    uint64_t start_block; // which chacha block this chunk starts at
} ChaChunkArg;
...
static int chacha20_xor_file_parallel(uint8_t *data, size_t data_len,
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        args[i].key = key;
        args[i].nonce = nonce;
        args[i].start_block = (offset / 64); // starting ChaCha block index for this chunk
        // create thread
        if (this_len > 0) {
...
// -----------------------------
// Utilities: generate random nonce 12 bytes
// -----------------------------
...
// -----------------------------
// High-level helpers para cifrar/descifrar archivos (usados por main):
// - chacha20_encrypt_file(inpath, outpath, passphrase, num_threads)
// - chacha20_decrypt_file(inpath, outpath, passphrase, num_threads)
// 
// Format used:
//   - Encriptado: [ 12 bytes nonce ][ ciphertext bytes ]
//   - Decrypt: lee nonce (12 bytes) y luego descifra.
// -----------------------------
...
static int chacha20_encrypt_file(const char *inpath, const char *outpath,
    ...
    if (generate_random_nonce(nonce) != 0) {
        // fallback: nonce = zeros (menos seguro)
        memset(nonce, 0, 12);
    }

    // cifra en memoria (XOR)
    if (chacha20_xor_file_parallel(buf, buf_len, key, nonce, num_threads) != 0) {
        free(buf); return -1;
    }

    // abrir archivo de salida y escribir nonce + ciphertext
...
static int chacha20_decrypt_file(const char *inpath, const char *outpath,
    ...
    if (total_len < 12) { free(buf); return -1; } // formato inválido
...
// =============================
//  BLOQUE 3 — FILE JOB STRUCT
// =============================

struct FileJob {
    string inpath;
    string outpath;
    int mode;          // 0 = compresión, 1 = descompresión, 2 = encrypt, 3 = decrypt
    string pass;       // para -e / -u
    int num_threads;
};
...
static void* file_worker(void *arg) {
    ...
    int is_lzw_mode = (job->mode == 0 || job->mode == 1);
    int is_enc_mode = (job->mode == 2 || job->mode == 3);

    // Crear fd entrada/salida
    ...
    if (job->mode == 1) {
        ...
        string base = strip_extension(strip_extension(fname)); // archivo.txt.lzw → archivo
        string ext  = get_extension(strip_extension(fname));   // archivo.txt.lzw → .txt
        if (ext == "") ext = ".out";
    ...
    if (job->mode == 3) {
        ...
        string base = strip_extension(strip_extension(fname)); // quita .enc
        string ext  = get_extension(strip_extension(fname)); // obtiene extensión original
        if (ext == "") ext = ".out";
...
// =============================
//  BLOQUE 4 — PROCESADO DIRECTORIOS
// =============================
                             ...
                             int mode, const string &pass, int num_threads)
{
    ...
    while ((ent = readdir(dp)) != NULL) {
        ...
        job->inpath = full_in;
        job->outpath = outdir;   // el worker arma el nombre final
        job->mode = mode;
        job->pass = pass;
...
int main(int argc, char **argv) {
    ...
    // ----------------------------
    if (mode < 0) {
        cerr << "Error: No se especificó modo -c | -d | -e | -u\n";
        return 1;
    }
    ...
    if (mode <= 1 && comp_alg == "") {
        cerr << "Error: Falta --comp_alg para compresión/descompresión\n";
        return 1;
    }

    if (mode <= 1 && comp_alg != "lzw") {
        cerr << "Error: --comp-alg debe ser lzw\n";
        return 1;
    }

    if ((mode == 2 || mode == 3) && enc_alg == "") {
        cerr << "Error: Falta --enc_alg para cifrado/descifrado\n";
        return 1;
    }

    if ((mode == 2 || mode == 3) && enc_alg != "chacha20") {
        cerr << "Error: --enc-alg debe ser lzw\n";
        return 1;
    }

    if ((mode == 2 || mode == 3) && k == "") {
        cerr << "Error: Falta -k para cifrado/descifrado\n";
        return 1;
    }
    ...
    // ----------------------------
    // Ruta input: archivo o directorio
    // ----------------------------
    ...
    if (is_file(inpath.c_str())) {
        ...
        process_directory(inpath, outpath, mode, k, num_threads);
    }
